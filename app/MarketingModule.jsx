...
export default function MarketingModule({data={records:[]},persistData,inventoryData={records:[]},geoData={sales:[]},bdcRecords=[],industryData=null,amdaData=null,branchCatalog=[],selectedMonth,canCapture=false,showToast}){
  ...
  return <div className="marketing-module"><style>{CSS}</style><style>{BRANCH_MULTI_CSS}</style>
    ...
    {activeTab==='campaigns'&&<section className="marketing-campaigns">
      <header className="section-title"><div><span className="eyebrow">INTELIGENCIA DE INVERSIÓN · BDC</span><h2>Rentabilidad de campañas propias</h2><p>Relaciona Meta Ads, campañas creadas, cierres trazados, inventario y ventas del modelo. Los datos se leen directamente de BDC y no se duplican.</p></div><span className="scope-chip">{selectedPeriodLabel} · {branchSelectionLabel(branch,ALL_BRANCHES)}</span></header>
