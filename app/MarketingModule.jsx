...
import BranchMultiSelect, { BRANCH_MULTI_CSS, branchSelectionLabel, branchSelectionMatches, branchSelectionValues } from './BranchMultiSelect';
...
export default function MarketingModule({data={records:[]},persistData,inventoryData={records:[]},geoData={sales:[]},bdcRecords=[],persistBdcData=null,industryData=null,amdaData=null,branchCatalog=[],selectedMonth,canCapture=false,showToast}){
  const activeBranches=useMemo(()=>branchCatalog.filter(item=>item?.active!==false&&key(item?.type)!=='SEMINUEVOS'),[branchCatalog]);
  const periods=useMemo(()=>{
    const values=new Set([selectedMonth,...(data.records||[]).map(row=>row.period),...(data.strategyRecords||[]).map(row=>row.period),...(data.competitionOffers||[]).map(row=>row.period),...(inventoryData.records||[]).map(row=>row.period),...(geoData?.sales||[]).map(row=>String(row.fecha||'').slice(0,7)),...(industryData?.bm||[]).map(row=>row[0]),...(amdaData?.r||[]).map(row=>row[1])].filter(value=>/^\d{4}-\d{2}$/.test(value)));
  ...
  useEffect(()=>{if(branchSelectionValues(branch,ALL_BRANCHES).some(name=>!brandBranches.some(item=>key(item.name)===key(name))))setBranch(ALL_BRANCHES);},[brand,brandBranches,branch]);
  ...
  const scopedManual=useMemo(()=>{
    ...
    const selectedBranches=branchSelectionValues(branch,ALL_BRANCHES);
    const exact=!selectedBranches.length?rows.filter(row=>key(row.branch)===key(ALL_BRANCHES)):[];
    if(exact.length)return exact;
    const scoped=selectedBranches.length?rows.filter(row=>branchSelectionMatches(branch,row.branch,ALL_BRANCHES)):rows;
    const grouped=new Map();
    scoped.forEach(row=>{const model=canonicalModel(row.model,catalog);const current=grouped.get(key(model))||{model,prospects:0,target:0};current.prospects+=Number(row.prospects)||0;current.target+=Number(row.target)||0;grouped.set(key(model),current);});
  ...
  const selectedBranchValues=branchSelectionValues(branch,ALL_BRANCHES);
  const multiBranchSelection=selectedBranchValues.length>1;
  const branchMatches=name=>selectedBranchValues.length===0
    ? key(name)===key(ALL_BRANCHES)||brandBranches.some(item=>key(item.name)===key(name))
    : branchSelectionMatches(branch,name,ALL_BRANCHES);
  const inventoryByModel=useMemo(()=>{
    let eligible=(inventoryData.records||[]).filter(row=>row.period<=period&&branchMatches(row.branch));
    if(selectedBranchValues.length===0){
  ...
  const save=async()=>{
    setSaving(true);try{
      if(multiBranchSelection)throw new Error('Para guardar prospectos o metas elige una sola sucursal o Todas las sucursales.');
  ...
  const campaignAnalysis=useMemo(()=>{
    ...
    const branchAllowed=value=>branchSelectionMatches(branch,value,ALL_BRANCHES);
    ...
    const results=selected.flatMap(record=>(record?.campaignResults||[]).filter(item=>key(item?.origin||item?.source)==='PROPIOS'&&key(item?.brand||'TORO')===brand&&((item?.branches||[]).length===0||selectedBranchValues.length===0||(item.branches||[]).some(branchAllowed))));
  ...
  const importMetaReport=async event=>{
    const file=event.target.files?.[0];event.target.value='';if(!file||!persistBdcData)return;
    setMetaImporting(true);
    try{
      const XLSX=await import('xlsx'),book=XLSX.read(await file.arrayBuffer(),{type:'array',cellDates:true}),sheet=book.Sheets[book.SheetNames[0]],source=XLSX.utils.sheet_to_json(sheet,{defval:'',raw:true}),stamp=new Date().toISOString();
      const selectedBranches=branchSelectionValues(branch,ALL_BRANCHES);
      const incoming=source.map((row,index)=>{
        const campaign=String(fieldValue(row,['Nombre de la campaña','Campaña'])||'').trim();
        const rawName=String(fieldValue(row,['Nombre del conjunto de anuncios','Conjunto de anuncios','Nombre del anuncio'])||campaign).trim();
        if(!campaign&&!rawName)return null;
        const indicator=String(fieldValue(row,['Indicador de resultados','Indicador de resultado','Tipo de resultado'])||'').trim(),indicatorKey=key(indicator);
        const rowPeriod=parsePeriod(row)||period;
        const rowBrand=key(fieldValue(row,['Marca','Brand'])||(`${campaign} ${rawName}`.toUpperCase().includes('CHS')?'CHS':brand));
        const resultType=indicatorKey.includes('LEAD')?'lead':indicatorKey.includes('CONVERS')||indicatorKey.includes('MENSAJ')?'conversation':'other';
        return {id:`meta-marketing-${rowPeriod}-${index}-${Date.now()}`,period:rowPeriod,brand:rowBrand==='CHS'?'CHS':'TORO',origin:'PROPIOS',source:'PROPIOS',rawName,campaignName:campaign,linkedCampaign:campaign||rawName,campaign:campaign||rawName,resultIndicator:indicator,resultType,results:cleanNumber(fieldValue(row,['Resultados','Resultado'])),spend:cleanNumber(fieldValue(row,['Importe gastado (MXN)','Importe gastado','Gasto'])),impressions:cleanNumber(fieldValue(row,['Impresiones'])),reach:cleanNumber(fieldValue(row,['Alcance'])),clicks:cleanNumber(fieldValue(row,['Clics en el enlace','Clics (todos)','Clics'])),frequency:cleanNumber(fieldValue(row,['Frecuencia'])),cpm:cleanNumber(fieldValue(row,['CPM (costo por 1.000 impresiones)','CPM'])),branches:selectedBranches,sourceFile:file.name,importedAt:stamp};
      }).filter(Boolean);
      if(!incoming.length)throw new Error('No se encontraron campañas o conjuntos de anuncios en el reporte.');
      const next=[...(bdcRecords||[])];
      [...new Set(incoming.map(row=>row.period))].forEach(targetPeriod=>{
        let index=-1;next.forEach((record,i)=>{if(record.period===targetPeriod)index=i;});
        const base=index>=0?next[index]:{id:`bdc-meta-${Date.now()}-${targetPeriod}`,period:targetPeriod,fileName:'Resultados Meta desde Marketing',uploadedAt:stamp,sourceRows:[],teams:[],apvs:[],campaigns:[],campaignResults:[],appointments:[],leadAssignments:[]};
        const rows=incoming.filter(row=>row.period===targetPeriod),identities=new Set(rows.map(row=>`${row.period}|${row.brand}|${key(row.rawName)}|${key(row.resultIndicator)}`));
        const campaignResults=[...(base.campaignResults||[]).filter(row=>!identities.has(`${targetPeriod}|${row.brand||'TORO'}|${key(row.rawName)}|${key(row.resultIndicator)}`)),...rows];
        const updated={...base,uploadedAt:stamp,campaignResultsFileName:file.name,campaignResults};
        if(index>=0)next[index]=updated;else next.push(updated);
      });
      await persistBdcData(next.sort((a,b)=>String(a.period).localeCompare(String(b.period))));
      showToast?.(`${incoming.length} resultados de Meta importados desde Marketing.`,'success');
    }catch(error){showToast?.(error?.message||'No fue posible importar el reporte de Meta.','error');}finally{setMetaImporting(false);}
  };

  return <div className="marketing-module"><style>{CSS}</style><style>{BRANCH_MULTI_CSS}</style><style>{`.marketing-campaign-tools{display:flex;align-items:flex-end;flex-direction:column;gap:8px}.marketing-campaign-tools button{display:flex;align-items:center;gap:7px;min-height:38px;padding:0 14px;border:0;border-radius:9px;background:#2865df;color:#fff;font-size:11px;font-weight:900;white-space:nowrap;cursor:pointer}.marketing-campaign-tools button:disabled{cursor:wait;opacity:.65}@media(max-width:760px){.marketing-campaign-tools{width:100%;align-items:stretch}.marketing-campaign-tools button{justify-content:center}}`}</style>
    ...
    <nav className="marketing-tabs"><button className={activeTab==='planning'?'active':''} onClick={()=>setActiveTab('planning')}><Target size={15}/> Planeación por modelo</button><button className={activeTab==='strategies'?'active':''} onClick={()=>setActiveTab('strategies')}><Sparkles size={15}/> Estrategias</button><button className={activeTab==='campaigns'?'active':''} onClick={()=>setActiveTab('campaigns')}><BarChart3 size={15}/> Campañas propias</button><button className={activeTab==='competition'?'active':''} onClick={()=>setActiveTab('competition')}><Eye size={15}/> Competencia</button></nav>
    <section className={`marketing-filters ${activeTab==='campaigns'?'campaign-filter-mode':''}`}>
      ...
      <BranchMultiSelect label="Sucursal" value={branch} onChange={setBranch} allValue={ALL_BRANCHES} options={brandBranches}/>
    ...
    {activeTab==='strategies'&&<MarketingStrategies data={data} persistData={persistData} period={period} brand={brand} branch={branch} branches={brandBranches} models={discoveredModels} modelPerformance={rows} canCapture={canCapture} showToast={showToast} search={search}/>}
    {activeTab==='planning'&&<>
    {canCapture&&<section className="marketing-import"><div><span className="eyebrow">ACTUALIZACIÓN MASIVA</span><strong>Plantilla mensual de prospectos y metas</strong><small>Descarga el catálogo vigente, actualízalo y vuelve a importarlo. Las coincidencias se reemplazan automáticamente.</small></div><div className="marketing-import-actions"><button className="secondary" onClick={downloadTemplate} disabled={multiBranchSelection} title={multiBranchSelection?'Selecciona una sola sucursal para descargar una plantilla de captura.':''}><Download size={15}/> Descargar plantilla</button><button onClick={()=>fileInputRef.current?.click()} disabled={importing}><Upload size={15}/>{importing?'Importando…':'Importar Excel'}</button><input ref={fileInputRef} type="file" hidden accept=".xlsx,.xls" onChange={importTemplate}/></div></section>}
    ...
    <section className="marketing-panel"><header><div><span className="eyebrow">MATRIZ DE DECISIÓN</span><h2>Desempeño comercial por unidad</h2><p>Las ventas provienen de Geointeligencia y el inventario del módulo de Inventarios. Prospectos y metas se capturan aquí.</p></div>{canCapture&&!multiBranchSelection&&<button onClick={save} disabled={saving}><Save size={15}/>{saving?'Guardando…':'Guardar prospectos y metas'}</button>}</header>
      {canCapture&&<div className="marketing-add"><input value={customModel} onChange={e=>setCustomModel(e.target.value)} onKeyDown={e=>e.key==='Enter'&&addModel()} placeholder="Agregar otro modelo" disabled={multiBranchSelection}/><button onClick={addModel} disabled={multiBranchSelection}><Plus size={15}/> Agregar</button><small>{multiBranchSelection?'La selección múltiple es de consulta; elige una sola sucursal para capturar.':selectedBranchValues.length===0?'La captura se guardará como consolidado de todas las sucursales.':''}</small></div>}
      <div className="marketing-table"><table><thead><tr><th>Unidad</th><th>Prospectos</th><th>Inventario actual</th><th>Ventas<br/>{periodLabel(period)}</th><th>Conversión</th><th>Ventas<br/>{periodLabel(previousYearPeriod(period))}</th><th>Variación anual</th><th>Meta ventas</th><th>Avance</th><th>Brecha</th><th>Recomendación</th></tr></thead><tbody>{rows.map(row=><tr key={key(row.model)}><td><strong>{row.model}</strong></td><td>{canCapture&&!multiBranchSelection?<input type="number" min="0" value={draft[key(row.model)]?.prospects??0} onChange={e=>update(row.model,'prospects',e.target.value)}/>:row.prospects}</td><td>{row.inventory}</td><td><b>{row.sales}</b></td><td>{pct(row.conversion)}</td><td>{row.prior}</td><td><span className={`delta ${Number.isFinite(row.annual)?row.annual>=0?'up':'down':'neutral'}`}>{Number.isFinite(row.annual)?`${row.annual>=0?'↑':'↓'} ${Math.abs(row.annual).toFixed(1)}%`:'Sin base'}</span></td><td>{canCapture&&!multiBranchSelection?<input type="number" min="0" value={draft[key(row.model)]?.target??0} onChange={e=>update(row.model,'target',e.target.value)}/>:row.target}</td><td>{pct(row.attainment,0)}</td><td className={row.target-row.sales>0?'negative':'positive'}>{row.target?`${row.sales-row.target>=0?'+':''}${row.sales-row.target}`:'—'}</td><td><span className={`strategy ${row.strategy.tone}`}>{row.strategy.label}</span><small className="strategy-detail">{row.strategy.detail}</small></td></tr>)}</tbody><tfoot><tr><td>TOTAL</td><td>{summary.prospects}</td><td>{summary.inventory}</td><td>{summary.sales}</td><td>{pct(summary.conversion)}</td><td>{summary.prior}</td><td>{summary.prior?pct((summary.sales/summary.prior-1)*100):'Sin base'}</td><td>{summary.target}</td><td>{pct(summary.attainment,0)}</td><td>{summary.target?summary.sales-summary.target:'—'}</td><td>Lectura consolidada</td></tr></tfoot></table></div>
    </section>
    ...
    {activeTab==='campaigns'&&<section className="marketing-campaigns">
      <header className="section-title"><div><span className="eyebrow">INTELIGENCIA DE INVERSIÓN · BDC</span><h2>Rentabilidad de campañas propias</h2><p>Relaciona Meta Ads, campañas creadas, cierres trazados, inventario y ventas del modelo. Los datos se leen directamente de BDC y no se duplican.</p></div><div className="marketing-campaign-tools"><span className="scope-chip">{selectedPeriodLabel} · {branchSelectionLabel(branch,ALL_BRANCHES)}</span>{canCapture&&persistBdcData&&<><button type="button" onClick={()=>metaInputRef.current?.click()} disabled={metaImporting}><Upload size={15}/>{metaImporting?'Importando Meta…':'Importar reporte Meta'}</button><input ref={metaInputRef} hidden type="file" accept=".xlsx,.xls" onChange={importMetaReport}/></>}</div></header>
