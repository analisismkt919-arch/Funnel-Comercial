...
function installStorageBridge(accessToken: string, profile: any, supabase: any) {
  ...
  (window as any).storage = {
    async get(key: string, shared = true) {
      ...
      try {
        ...
        const values = await Promise.all(Array.from({ length: manifest.chunks }, async (_, index) => {
          const chunk = await readStorageValue(`${BDC_KEY}::chunk::${manifest.generation}::${index}`, shared);
    ...
    async set(key: string, value: string, shared = true) {
      ...
      if (key !== BDC_KEY || value.length <= STORAGE_CHUNK_SIZE) return writeStorageValue(key, value, shared);
      let previousGeneration = "";
      try {
        const current = await readStorageValue(BDC_KEY, shared);
      ...
      for (let index = 0; index < chunks; index += 1) {
        await writeStorageValue(`${BDC_KEY}::chunk::${generation}::${index}`, value.slice(index * STORAGE_CHUNK_SIZE, (index + 1) * STORAGE_CHUNK_SIZE), shared);
      }
      await writeStorageValue(BDC_KEY, JSON.stringify({ __chunkedStorage: 1, generation, chunks, length: value.length, updatedAt: new Date().toISOString() }), shared);
