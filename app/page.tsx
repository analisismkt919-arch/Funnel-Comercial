...
import { PLATFORM_NAME, PLATFORM_DESCRIPTOR, PLATFORM_TAGLINE } from "./brand";

const SESSION_KEY = "funnel-kpi-session-v1";
const RECORDS_KEY = "funnel-kpi-v2-records";
const BDC_KEY = "funnel-kpi-bdc-v1";
const BDC_BACKUP_KEY = `${BDC_KEY}::backup`;
const TELEMARKETING_KEY = `${BDC_KEY}::chunk::telemarketing`;
// Mantener cada respuesta por debajo de un tamaño cómodo para el navegador.
// BDC y Telemarketing pueden crecer mucho; fragmentos pequeños evitan picos de
// memoria durante el inicio de sesión y al volver a abrir esos módulos.
const STORAGE_CHUNK_SIZE = 1_000_000;
const isChunkedStorageKey = (key: string) => key === BDC_KEY || key === BDC_BACKUP_KEY || key === TELEMARKETING_KEY;
const TURNSTILE_SITE_KEY = process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY || "";
const profileRequiresMfa = (profile: any) =>
  profile?.role === "admin" ||
  profile?.role === "capturista" ||
  profile?.role === "bdc_operador" ||
  profile?.role === "gerente_bdc" ||
...
function installStorageBridge(accessToken: string, profile: any, supabase: any) {
  ...
  const readStorageValue = async (key: string, shared: boolean) => {
    const query = new URLSearchParams({ key, scope: shared ? "shared" : "personal" });
    return request(`/api/storage?${query}`);
  };
  const writeStorageValue = (key: string, value: string, shared: boolean) =>
    request("/api/storage", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ key, value, scope: shared ? "shared" : "personal" }) });
  (window as any).storage = {
    async get(key: string, shared = true) {
      ...
      try {
        const payload = await readStorageValue(key, shared);
        if (!isChunkedStorageKey(key)) return payload;
        const manifest = JSON.parse(payload?.value || "null");
        if (!manifest?.__chunkedStorage || !manifest.generation || !Number.isInteger(manifest.chunks)) return payload;
        if (manifest.chunks < 1 || manifest.chunks > 100) throw new Error("El respaldo BDC fragmentado es inválido");
        const values: string[] = [];
        // La lectura secuencial es deliberada: solicitar todos los fragmentos
        // en paralelo duplicaba el consumo de memoria y podía cerrar la pestaña.
        for (let index = 0; index < manifest.chunks; index += 1) {
          const chunk = await readStorageValue(`${key}::chunk::${manifest.generation}::${index}`, shared);
          values.push(String(chunk?.value || ""));
        }
        const value = values.join("");
        if (value.length !== manifest.length) throw new Error("La importación BDC está incompleta; vuelve a cargar el archivo");
        return { value };
      } catch (error: any) {
    ...
    async set(key: string, value: string, shared = true) {
      ...
      if (!isChunkedStorageKey(key) || value.length <= STORAGE_CHUNK_SIZE) return writeStorageValue(key, value, shared);
      let previousGeneration = "";
      try {
        const current = await readStorageValue(key, shared);
        previousGeneration = String(JSON.parse(current?.value || "null")?.generation || "");
      } catch {}
      // Dos juegos alternos permiten publicar el manifiesto al final sin dejar
      // generaciones ilimitadas ocupando espacio en Supabase.
      const generation = previousGeneration === "a" ? "b" : "a";
      const chunks = Math.ceil(value.length / STORAGE_CHUNK_SIZE);
      for (let index = 0; index < chunks; index += 1) {
        await writeStorageValue(`${key}::chunk::${generation}::${index}`, value.slice(index * STORAGE_CHUNK_SIZE, (index + 1) * STORAGE_CHUNK_SIZE), shared);
      }
      await writeStorageValue(key, JSON.stringify({ __chunkedStorage: 1, generation, chunks, length: value.length, updatedAt: new Date().toISOString() }), shared);
      return { ok: true, chunked: true, chunks };
    },
...
export default function Home() {
  ...
  if (!ready) return <div className="bridge-loading"><img src="/logo-cr3-drive.png" alt={PLATFORM_NAME} /><strong>{PLATFORM_NAME}</strong><span>Preparando tu inteligencia comercial…</span></div>;
  if (!authenticated && mfaStage !== "none") return (
    <main className="supabase-login">
      <form onSubmit={verifyMfa} className="mfa-form">
        <div className="login-logos">
          <img className="login-cr3-logo" src="/logo-cr3-drive.png" alt={PLATFORM_NAME} />
          <span aria-hidden="true" />
          <img className="login-brand-logos" src="/logos-marcas-negros.png" alt="Chevrolet, Buick, GMC y Cadillac" />
        </div>
        <div className="login-eyebrow">{PLATFORM_NAME} · Verificación de seguridad</div>
    ...
    <main className="supabase-login">
      <form onSubmit={login}>
        ...
        <div className="login-product">
          <div className="login-eyebrow">{PLATFORM_DESCRIPTOR}</div>
          <h1>CR3 Drive <span>Intelligence 360</span></h1>
          <p>{PLATFORM_TAGLINE}</p>
        </div>
        <h2 className="login-access-title">Iniciar sesión</h2>
        <p className="login-intro">Accede a la visión integral de ventas, marketing, inventarios, industria y geointeligencia.</p>
        <label>
          Correo electrónico
