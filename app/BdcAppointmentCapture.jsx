...
export default function BdcAppointmentCapture({mode='create',appointments=[],globalPeriod='',catalogTeams=[],branchCatalog=[],dealerRecords=[],campaignCatalog=[],onSave,onDelete,canDelete=false,showToast}){
  ...
  return <div className={`bdc-appointment-module ${globalPeriod?'uses-global-period':''}`}>
    ...
    {mode==='followup'&&<>
      ...
      {followupView==='calendar'&&<section className="bdc-appointment-calendar"><div className="bdc-calendar-head"><div><CalendarDays size={20}/><div><h3>Calendario de citas · {MONTHS[Number(calendarMonth)-1]} {calendarYear}</h3><p>Selecciona una cita para consultar cliente, responsables y unidad de interés.</p></div></div><div className="bdc-calendar-controls"><label>Año<select value={calendarYear} onChange={event=>setCalendarPeriod(`${event.target.value}-${calendarMonth}`)}>{calendarYears.map(year=><option key={year}>{year}</option>)}</select></label><label>Mes<select value={calendarMonth} onChange={event=>setCalendarPeriod(`${calendarYear}-${event.target.value}`)}>{MONTHS.map((month,index)=>{const value=String(index+1).padStart(2,'0');return <option key={month} value={value}>{month}</option>})}</select></label><div className="bdc-calendar-count"><strong>{calendarRows.length}</strong><span>{calendarRows.length===1?'cita':'citas'}</span></div></div></div><div className="bdc-calendar-weekdays">{['Lun','Mar','Mié','Jue','Vie','Sáb','Dom'].map(day=><span key={day}>{day}</span>)}</div><div className="bdc-calendar-grid">{calendarCells.map((day,index)=><div className={`bdc-calendar-day ${day?'':'outside'} ${index%7>4?'weekend':''}`} key={`${day||'empty'}-${index}`}>{day&&<><div className="bdc-calendar-day-number"><b>{day}</b>{appointmentsByDay[day]?.length?<span>{appointmentsByDay[day].length}</span>:null}</div><div className="bdc-calendar-events">{(appointmentsByDay[day]||[]).map(row=><button type="button" key={row.id||`${row.fechaCita}-${row.hora}-${row.cliente}`} onClick={()=>setSelectedCalendarAppointment(row)}><time>{row.hora||'--:--'}</time><span>{row.cliente||'Prospecto sin nombre'}</span></button>)}</div></>}</div>)}</div>{!calendarRows.length&&<div className="bdc-calendar-empty"><CalendarClock size={22}/><span>No hay citas agendadas para este mes.</span></div>}</section>}
