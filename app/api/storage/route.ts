...
export async function POST(request: Request) {
  ...
  if (key === RECORDS_KEY) {
    ...
    const { error } = await auth.admin.rpc("replace_funnel_records", { p_records: rows, p_user_id: auth.user.id });
    if (error) return Response.json({ error: error.message }, { status: 400 });
    await audit(auth.admin, auth.user.id, "RECORDS_REPLACED", key, { count: rows.length });
    return Response.json({ ok: true });
  }
