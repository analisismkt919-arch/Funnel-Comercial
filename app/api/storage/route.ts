...
export async function POST(request: Request) {
  ...
  if (key === RECORDS_KEY) {
    ...
    // Reemplazo físico sin depender de funciones SQL ni extensiones.
    // Se escribe por bloques para no exceder el límite de las funciones de Vercel.
    ...
    for (let start = 0; start < databaseRows.length; start += 400) {
      const { error: insertError } = await auth.admin
        .from("funnel_records")
        .insert(databaseRows.slice(start, start + 400));
      if (insertError) {
