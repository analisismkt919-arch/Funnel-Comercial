...
const FI_KEY = "funnel-kpi-fi-v1";
const BDC_KEY = "funnel-kpi-bdc-v1";
const BDC_STAFF_KEY = "funnel-kpi-bdc-staff-v1";
const TELEMARKETING_KEY = `${BDC_KEY}::chunk::telemarketing`;
const CHUNKED_ROOT_KEYS = new Set([BDC_KEY, TELEMARKETING_KEY]);
const SECURITY_ALERTS_KEY = "security-alerts";
const ADMIN_KEYS = new Set([USERS_KEY, BDC_STAFF_KEY, "funnel-kpi-v2-targets", "funnel-kpi-period-locks-v1", "funnel-kpi-v2-branches"]);
const CAPTURE_KEYS = new Set([RECORDS_KEY, "funnel-kpi-v2-teams", BDC_KEY, "funnel-kpi-fi-v1", "funnel-kpi-industry-v1", "funnel-kpi-amda-v1", "funnel-kpi-industry-preliminary-v1"]);
const permissionKey = (key: string) => key.startsWith(`${BDC_KEY}::chunk::`) ? BDC_KEY : key;

const isAdmin = (profile: any) => profile.role === "admin";
const canCapture = (profile: any) => isAdmin(profile) || profile.role === "capturista" || profile.role === "bdc_operador" || profile.role === "gerente_bdc" || profile.permissions?.canCapture === true;
...
function chunkManifest(value: string) {
  const parsed = jsonValue(value);
  if (!parsed?.__chunkedStorage || !["a", "b"].includes(String(parsed.generation))) return null;
  const chunks = Number(parsed.chunks);
  if (!Number.isInteger(chunks) || chunks < 1 || chunks > 100) return null;
  return { generation: String(parsed.generation), chunks };
}

async function purgeObsoleteStorageChunks(admin: any, storageKey: string, scope: string, ownerId: string | null, value: string) {
  if (!CHUNKED_ROOT_KEYS.has(storageKey)) return 0;
  let query = admin.from("funnel_storage").select("storage_key")
    .eq("scope", scope)
    .like("storage_key", `${storageKey}::chunk::%`);
  query = ownerId ? query.eq("owner_id", ownerId) : query.is("owner_id", null);
  const { data, error } = await query;
  if (error) throw error;

  const manifest = chunkManifest(value);
  const strictChunk = new RegExp(`^${storageKey.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}::chunk::([ab])::(\\d+)$`);
  const obsolete = (data || []).map((row: any) => String(row.storage_key || "")).filter((candidate: string) => {
    const match = candidate.match(strictChunk);
    if (!match) return false;
    if (!manifest) return true;
    return match[1] !== manifest.generation || Number(match[2]) >= manifest.chunks;
  });
  for (let start = 0; start < obsolete.length; start += 100) {
    let remove = admin.from("funnel_storage").delete().in("storage_key", obsolete.slice(start, start + 100)).eq("scope", scope);
    remove = ownerId ? remove.eq("owner_id", ownerId) : remove.is("owner_id", null);
...
export async function GET(request: Request) {
  ...
  if (key === FI_KEY && !isAdmin(auth.profile)) {
    const access = await recordScope(auth.admin, auth.profile);
    const parsed = jsonValue(data.value) || {};
    const filtered = {
      records: (Array.isArray(parsed.records) ? parsed.records : []).filter((row: any) => scopeAllows(access, row.branch, row.manager)),
      targets: (Array.isArray(parsed.targets) ? parsed.targets : []).filter((row: any) => scopeAllows(access, row.branch)),
      productivity: (Array.isArray(parsed.productivity) ? parsed.productivity : []).filter((row: any) => scopeAllows(access, row.branch)),
      penetration: (Array.isArray(parsed.penetration) ? parsed.penetration : []).filter((row: any) => scopeAllows(access, row.branch)),
      productivityProjects: Array.isArray(parsed.productivityProjects) ? parsed.productivityProjects : []
    };
    return Response.json({ value: JSON.stringify(filtered) });
  }
...
export async function POST(request: Request) {
  ...
  const securedKey = permissionKey(key);
  if (scope === "shared" && ADMIN_KEYS.has(securedKey) && !isAdmin(auth.profile)) return Response.json({ error: "Solo un administrador puede modificar esta información" }, { status: 403 });
  if (scope === "shared" && CAPTURE_KEYS.has(securedKey) && !canCapture(auth.profile)) return Response.json({ error: "Tu perfil no tiene permiso de captura" }, { status: 403 });

  if (key === FI_KEY) {
    const incoming = jsonValue(value);
    if (!incoming || !Array.isArray(incoming.records) || !Array.isArray(incoming.targets)) return Response.json({ error: "Estructura F&I inválida" }, { status: 400 });
    incoming.productivity = Array.isArray(incoming.productivity) ? incoming.productivity : [];
    incoming.penetration = Array.isArray(incoming.penetration) ? incoming.penetration : [];
    incoming.productivityProjects = Array.isArray(incoming.productivityProjects) ? incoming.productivityProjects.map((value:any)=>String(value).trim()).filter(Boolean).slice(0,30) : [];
    const validMonth = (row: any) => /^\d{4}-(0[1-9]|1[0-2])$/.test(String(row?.month || ""));
    if ([...incoming.productivity,...incoming.penetration].some((row: any) => !validMonth(row) || !String(row.branch || "").trim())) return Response.json({ error: "Resultados contiene periodos o sucursales inválidos" }, { status: 400 });
    if (incoming.records.some((row: any) => !validMonth(row) || !String(row.branch || "").trim() || !String(row.manager || "").trim()) || incoming.targets.some((row: any) => !validMonth(row) || !String(row.branch || "").trim())) return Response.json({ error: "F&I contiene periodos, sucursales o gerentes inválidos" }, { status: 400 });
    if (incoming.records.length > 10000 || incoming.targets.length > 5000) return Response.json({ error: "F&I excede el límite de registros permitido" }, { status: 400 });
    if (!isAdmin(auth.profile)) {
      const access = await recordScope(auth.admin, auth.profile);
      const invalid = incoming.records.some((row: any) => !scopeAllows(access,row.branch,row.manager)) || [...incoming.targets,...incoming.productivity,...incoming.penetration].some((row: any) => !scopeAllows(access,row.branch));
      if (invalid) { await audit(auth.admin,auth.user.id,"ACCESS_DENIED",key,{reason:"FI_SCOPE"}); return Response.json({error:"Hay registros F&I fuera de tu alcance"},{status:403}); }
      const { data: stored } = await auth.admin.from("funnel_storage").select("value").eq("storage_key",key).eq("scope",scope).is("owner_id",null).maybeSingle();
      const current = jsonValue(stored?.value) || {records:[],targets:[]};
      incoming.records = [...(Array.isArray(current.records)?current.records:[]).filter((row:any)=>!scopeAllows(access,row.branch,row.manager)),...incoming.records];
      incoming.targets = [...(Array.isArray(current.targets)?current.targets:[]).filter((row:any)=>!scopeAllows(access,row.branch)),...incoming.targets];
      incoming.productivity = [...(Array.isArray(current.productivity)?current.productivity:[]).filter((row:any)=>!scopeAllows(access,row.branch)),...incoming.productivity];
      incoming.penetration = [...(Array.isArray(current.penetration)?current.penetration:[]).filter((row:any)=>!scopeAllows(access,row.branch)),...incoming.penetration];
    }
    const fiValue=JSON.stringify(incoming);
    const { error } = await auth.admin.from("funnel_storage").upsert({storage_key:key,scope,owner_id:owner(auth.profile,scope),value:fiValue,updated_by:auth.user.id,updated_at:new Date().toISOString()},{onConflict:"storage_key,scope,owner_key"});
    if(error)return Response.json({error:error.message},{status:400});
    await audit(auth.admin,auth.user.id,"FI_STORAGE_UPSERTED",key,{records:incoming.records.length,targets:incoming.targets.length});
  ...
  const { error } = await auth.admin.from("funnel_storage").upsert({
    storage_key: key, scope, owner_id: ownerId, value, updated_by: auth.user.id, updated_at: new Date().toISOString()
  }, { onConflict: "storage_key,scope,owner_key" });
  if (error) return Response.json({ error: error.message }, { status: 400 });
  let purgedChunks = 0;
  try {
    purgedChunks = await purgeObsoleteStorageChunks(auth.admin, key, scope, ownerId, value);
  } catch (cleanupError: any) {
    await audit(auth.admin, auth.user.id, "STORAGE_CHUNK_CLEANUP_FAILED", key, { error: cleanupError?.message || String(cleanupError) });
    // El valor principal ya quedó confirmado. Un fallo al retirar fragmentos
    // obsoletos no debe hacer que el cliente interprete la escritura como
    // fallida y vuelva a mostrar una versión anterior.
    await audit(auth.admin, auth.user.id, "STORAGE_UPSERTED", key, { cleanupWarning: true });
    return Response.json({ ok: true, purgedChunks: 0, cleanupWarning: true });
  }
  await audit(auth.admin, auth.user.id, "STORAGE_UPSERTED", key);
  if (purgedChunks) await audit(auth.admin, auth.user.id, "OBSOLETE_STORAGE_CHUNKS_PURGED", key, { count: purgedChunks });
  return Response.json({ ok: true, purgedChunks });
}
...
export async function DELETE(request: Request) {
  ...
  if (key === RECORDS_KEY) {
    ...
    const ownerId = owner(auth.profile, scope);
    if (CHUNKED_ROOT_KEYS.has(key)) {
      let chunkQuery = auth.admin.from("funnel_storage").select("storage_key").eq("scope", scope).like("storage_key", `${key}::chunk::%`);
      chunkQuery = ownerId ? chunkQuery.eq("owner_id", ownerId) : chunkQuery.is("owner_id", null);
      const { data: chunkRows, error: chunkReadError } = await chunkQuery;
      if (chunkReadError) return Response.json({ error: chunkReadError.message }, { status: 400 });
      const strictChunk = new RegExp(`^${key.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}::chunk::[ab]::\\d+$`);
      const chunkKeys = (chunkRows || []).map((row: any) => String(row.storage_key || "")).filter((candidate: string) => strictChunk.test(candidate));
      if (chunkKeys.length) {
        let chunkDelete = auth.admin.from("funnel_storage").delete().in("storage_key", chunkKeys).eq("scope", scope);
        chunkDelete = ownerId ? chunkDelete.eq("owner_id", ownerId) : chunkDelete.is("owner_id", null);
        const { error: chunkDeleteError } = await chunkDelete;
        if (chunkDeleteError) return Response.json({ error: chunkDeleteError.message }, { status: 400 });
      }
    }
