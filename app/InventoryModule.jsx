...
export default function InventoryModule({
  ...
  const visibleRows = useMemo(() => draft.filter(row => {
    if (branch !== 'todas' && textKey(row.branch)!==textKey(ALL_BRANCHES) && !branchSelectionMatches(branch,row.branch,'todas')) return false;
  ...
  const addRow = () => setDraft(rows => [...rows, makeRow(period, branchSelectionValues(branch,'todas')[0] || ALL_BRANCHES)]);
  ...
  return <div className="inventory-module">
    ...
    <section className="inventory-filters">
      <BranchMultiSelect label="Sucursal" value={branch} onChange={setBranch} allValue="todas" options={allowedBranches}/>
    ...
    <section className="inventory-panel">
      <header><div><span className="inventory-eyebrow">RESUMEN EJECUTIVO</span><h2>Antigüedad por modelo</h2></div><small>{periodLabel(period)} · {branchSelectionLabel(branch,'todas')}</small></header>
