...
import { AlertTriangle, Boxes, Car, CheckCircle2, Plus, Save, Search, Trash2 } from 'lucide-react';
...
export default function InventoryModule({
  ...
  const allowedKeys = useMemo(() => new Set(allowedBranches.map(item => textKey(item.name))), [allowedBranches]);
  useEffect(() => {
  ...
  const visibleRows = useMemo(() => draft.filter(row => {
    if (branch !== 'todas' && textKey(row.branch) !== textKey(branch)) return false;
  ...
  const addRow = () => setDraft(rows => [...rows, makeRow(period, branch === 'todas' ? allowedBranches[0]?.name : branch)]);
  ...
  return <div className="inventory-module">
    ...
    <section className="inventory-hero">
      ...
      <div className="inventory-period"><span>Periodo</span><select value={period} onChange={event => setPeriod(event.target.value)}>{availablePeriods.map(value => <option key={value} value={value}>{periodLabel(value)}</option>)}</select></div>
    </section>
...
.inventory-eyebrow{display:flex;align-items:center;gap:7px;color:#86b2ff;font-size:10px;font-weight:900;letter-spacing:.12em}.inventory-hero h1{margin:8px 0 5px;font-size:25px}.inventory-hero p,.inventory-panel header p{margin:0;color:#dbe8ff;font-size:12px}.inventory-period{display:grid;gap:5px;min-width:190px}.inventory-period span,.inventory-filters label>span{font-size:9px;font-weight:900;letter-spacing:.08em;text-transform:uppercase}.inventory-period select,.inventory-filters select,.inventory-filters input,.inventory-entry-table input,.inventory-entry-table select{height:39px;border:1px solid #d6e0ed;border-radius:9px;background:#fff;padding:0 11px;color:#102b57;font-weight:700;outline:none}.inventory-period select{border-color:rgba(255,255,255,.35);background:#fff;color:#102b57}
...
@media(max-width:900px){.inventory-hero{align-items:stretch;flex-direction:column}.inventory-filters{grid-template-columns:1fr}.inventory-kpis{grid-template-columns:repeat(2,1fr)}.inventory-panel>header{align-items:flex-start;flex-direction:column}.inventory-actions{width:100%;flex-wrap:wrap}}
