...
with existing_auth_users as (
  select
    user_row.*,
    row_number() over (order by user_row.created_at, user_row.id) as position
...
select
  ...
  coalesce(user_row.raw_user_meta_data->>'name', split_part(coalesce(user_row.email,''),'@',1)),
  case
    when not exists (select 1 from public.funnel_profiles) and user_row.position = 1 then 'admin'
    else 'capturista'
  end,
  true
from existing_auth_users user_row
on conflict(id) do nothing;
