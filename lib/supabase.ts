...
export async function authenticatedProfile(request: Request) {
  ...
  const aal = String((claimsData?.claims as any)?.aal || "aal1");
  ...
  const { data: profile } = await admin
    ...
    .eq("id", data.user.id)
    .maybeSingle();
  // Fail closed: una cuenta de Auth sin perfil autorizado no obtiene acceso.
  // Los perfiles deben crearse exclusivamente desde la administración.
  if (!profile || profile.active === false) return null;
  return { user: data.user, profile, admin, aal };
}
